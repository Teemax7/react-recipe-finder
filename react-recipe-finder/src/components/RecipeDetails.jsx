...
export default function RecipeDetails({ onToggleFavorite }) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-2xl shadow-lg">
      <img
        src={recipe.strMealThumb || "https://via.placeholder.com/600x400"}
        alt={recipe.strMeal}
        className="w-full rounded-xl mb-6"
      />
      <h1 className="text-3xl font-bold text-green-600 mb-4">{recipe.strMeal}</h1>
      <button
        onClick={() => onToggleFavorite(recipe)}
        className="mb-6 px-4 py-2 rounded-xl font-medium bg-green-500 hover:bg-green-600 text-white"
      >
        Add / Remove Favorite
      </button>
